...
export default async (request, context) => {
  ...
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/flash_sales?slug=eq.${encodeURIComponent(slug)}&select=titulo`,
      { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } }
    );
    const data = await res.json();
    const flash = data && data[0];
    if (!flash || !flash.titulo) return response;

    const tituloCompleto = escapeHtml(`⚡ ${flash.titulo} — Jar Jar Green`);
...
export const config = { path: "/flash-sale-en-vivo.html" };
